...
async def save_photo(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(text="Что делаем?", reply_markup=choice)
...
async def answer(call: types.CallbackQuery, state: FSMContext):
    ...
    choice = create_choice()
    if list_photos == 0:
        await bot.send_message(ids, "У вас нет фотографий")  # Если вместо списка получили ложный вызов
    else:
        try:
    ...
    await bot.send_message(ids, text="Что делаем?", reply_markup=choice)
